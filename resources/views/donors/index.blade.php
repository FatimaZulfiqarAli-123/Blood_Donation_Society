...
<script>
    $(document).ready(function() {
        ...
        $('form[id^="message-form"]').on('submit', function(event) {
            ...
            var donorId = formId.replace('message-form', '');

            var csrfToken = $('meta[name="csrf-token"]').attr('content');
            var message = $('#message' + donorId).val();

            $.ajax({
                url: `{{ route('messages.store') }}`,
                type: 'POST',
                dataType: 'json',
                data: { message: message, id: donorId },
                headers: {
                ...
                success: function(response) {
                    if(response.status == 200) {
                        ...
                        $('#messageModal' + donorId).modal('hide');
...
@role('patient')
    <div class="alert-container">
        <div class="alert alert-warning alert-dismissible text-white fade show" role="alert">
            ...
            <span class="alert-text"><strong>Note!</strong> Donors in a 'sleeping' state are temporarily unable to donate blood; please check back later or choose another donor. </span>
            <button type="button" class="btn-close" data-bs-dismiss="alert" aria-label="Close">
                <span aria-hidden="true">&times;</span>
            </button>
        </div>
...
<div class="container-fluid mt-3 mb-3" style="background-color: white; border-radius: 10px; align: center;">
    <form action="{{ route('donors.index') }}" method="POST" id="searchForm">
        @csrf
        @method('GET')
        <div class="row">
            ...
            <div class="card my-4">
                <div class="card-header p-0 position-relative mt-n4 mx-3 z-index-2">
                    <div class="bg-gradient-primary shadow-primary border-radius-lg pt-4 pb-3">
                        <h6 class="text-white text-capitalize ps-3">Donors List</h6>
                    </div>
                </div>
                <div class="card-body px-0 pb-2">
                    <div class="table-responsive p-0">
                        <table class="table align-items-center mb-0">
                            <thead>
                                <tr>
                                    <th class="text-center text-uppercase text-secondary text-xxs font-weight-bolder">Donor</th>
                                    ...
                                    @role('patient')<th class="text-center text-uppercase text-secondary text-xxs font-weight-bolder">Request</th>@endrole
                                </tr>
                            </thead>
                            <tbody>
                                @foreach ($donors as $donor)
                                    <tr>
                                            ...
                                            <div class="d-flex px-2 py-1">
                                                <div>
                                                    @if ($donor->profile_picture)
                                                        <img src="{{ asset('storage/' . $donor->profile_picture) }}" class="avatar avatar-sm me-3 border-radius-lg" alt="profile picture">
                                                    @else
                                                        <img src="{{ asset('assets/img/profile_picture_placeholder.jpg') }}" alt="profile_image" class="avatar avatar-sm me-3">
                                                    @endif
                                                </div>
                                                <div class="d-flex flex-column justify-content-center">
                                                    <h6 class="text-xs mb-0">{{ $donor->name }}</h6>
                                                </div>
                                            </div>
                                        </td>
                                        <td class="align-middle text-center">
                                            <h6 class="text-xs mb-0">{{ $donor->phone }}</h6>
                                        </td>
                                        <td class="align-middle text-center">
                                            <h6 class="text-xs mb-0">{{ $donor->email }}</h6>
                                        </td>
                                        <td class="align-middle text-center">
                                            <h6 class="text-xs mb-0">{{ $donor->city }}</h6>
                                        </td>
                                        <td class="align-middle text-center">
                                            <h6 class="text-xs mb-0">{{ $donor->blood_group }}</h6>
                                        </td>
                                        <td class="align-middle text-center">
                                            @if($donor->status != 'blocked')
                                                <span class="badge badge-sm {{ $donor->isEligibleForDonation() ? 'bg-gradient-success' : 'bg-gradient-warning' }}">{{ $donor->isEligibleForDonation() ? 'Active' : 'Sleeping' }}</span>
                                            @else
                                                <span class="badge badge-sm bg-gradient-danger">{{ $donor->status }}</span>
                                            @endif
                                        </td>
                                        @role('admin')
                                            <td class="align-middle text-center">
                                                <a href="" data-bs-toggle="modal" data-bs-target="#deleteModal{{ $donor->id }}">
                                                    <i class="material-icons text-md mb-0 position-relative">delete</i>
                                                </a>
                                                <a href="" data-bs-toggle="modal" data-bs-target="#blockModal{{ $donor->id }}">
                                                    <i class="material-icons text-md mb-0 position-relative">block</i>
                                                </a>
                                                <a href="" data-bs-toggle="modal" data-bs-target="#messageModal{{ $donor->id }}">
                                                    <i class="material-icons text-md mb-0 position-relative">message</i>
                                                </a>
                                            </td>

                                            <div class="modal fade mt-6" id="deleteModal{{ $donor->id }}" tabindex="-1" aria-labelledby="deleteModalLabel" aria-hidden="true">
                                                <div class="modal-dialog modal-custom">
                                                    <div class="modal-content">
                                                        <div class="modal-body">
                                                            <h5>Do you want to delete this Donor</h5>
                                                            <div>
                                                                <button type="button" class="btn btn-secondary" data-bs-dismiss="modal">Cancel</button>
                                                                <button type="button" class="btn btn-danger" onclick="document.getElementById('delete-form{{ $donor->id }}').submit();">Delete</button>
                                                            </div>
                                                            <form id="delete-form{{ $donor->id }}" action="{{ route('donor.delete', $donor->id) }}" method="POST" style="display: none;">
                                                                @csrf
                                                                @method('DELETE')
                                                            </form>
                                                        </div>
                                            ...
                                            <div class="modal fade mt-6" id="blockModal{{ $donor->id }}" tabindex="-1" aria-labelledby="blockModalLabel" aria-hidden="true">
                                                <div class="modal-dialog modal-custom">
                                                    <div class="modal-content">
                                                        <div class="modal-body">
                                                            <h5>Do you want to block this Donor</h5>
                                                            <div>
                                                                <button type="button" class="btn btn-secondary" data-bs-dismiss="modal">Cancel</button>
                                                                <button type="button" class="btn btn-danger" onclick="document.getElementById('block-form{{ $donor->id }}').submit();">Block</button>
                                                            </div>
                                                            <form id="block-form{{ $donor->id }}" action="{{ route('donor.block', $donor->id) }}" method="POST" style="display: none;">
                                                                @csrf
                                                                @method('PUT')
                                                            </form>
                                                        </div>
                                            ...
                                            <div class="modal fade mt-6" id="messageModal{{ $donor->id }}" tabindex="-1" aria-labelledby="messageModalLabel" aria-hidden="true">
                                                <div class="modal-dialog modal-custom">
                                                    <div class="modal-content">
                                                        <div class="modal-header">
                                                            Send Message to {{ $donor->name }}
                                                        </div>
                                                        <div class="modal-body">
                                                            <form id="message-form{{ $donor->id }}" method="POST">
                                                                @csrf
                                                                <textarea name="message" id="message{{ $donor->id }}" cols="56" rows="3" placeholder="Type your message here..."></textarea>
                                                                <input type="hidden" value="{{ $donor->id }}" id="donorId{{ $donor->id }}">
                                                                <div>
                                                                    <button type="button" class="btn btn-secondary" data-bs-dismiss="modal">Cancel</button>
                                        ...
                                        @role('patient')
                                            <td class="align-middle text-center">
                                                <a href="" data-bs-toggle="modal" data-bs-target="#sendRequestModal{{ $donor->id }}">
                                                    <button class="btn btn-danger">Request blood</button>
                                                </a>
                                            </td>

                                            <div class="modal fade mt-6" id="sendRequestModal{{ $donor->id }}" tabindex="-1" aria-labelledby="sendRequestModalLabel" aria-hidden="true">
                                                <div class="modal-dialog modal-custom">
                                                    <div class="modal-content">
                                                        <div class="modal-body">
                                                            <h5>Do you want to send blood request to this donor with following details?</h5>
                                                            <div class="card border shadow mb-3">
                                                                <div class="card-body">
                                                                    <div class="row">
                                                                        ...
                                                                        <div class="col-md-6 mb-2">
                                                                            <span class="fw-bold">Blood Group:</span> {{ $donor->blood_group }}
                                                                        </div>
                                                                    </div>
                                                                    <div class="row">
                                                                        <div class="col-md-6 mb-2">
                                                            ...
                                                            <form id="send-request-form{{ $donor->id }}" action="{{ route('blood.request') }}" method="POST" style="display: none;">
                                                                @csrf
                                                                <input type="hidden" name="requester_id" value="{{ auth()->id() }}">
                                                                <input type="hidden" name="receiver_id" value="{{ $donor->id }}">
                                                                <input type="hidden" name="blood_group" value="{{ $donor->blood_group }}">
                                                            </form>
                                                            <div>
                                                                <button type="button" class="btn btn-secondary" data-bs-dismiss="modal">Cancel</button>
                                                                <button type="button" class="btn btn-danger" onclick="document.getElementById('send-request-form{{ $donor->id }}').submit();">Send</button>
                                                            </div>
                                                        </div>
                                                    </div>
                                                </div>
